...
export function JobCard({
  ...
  const openDetails = () => onExpand(job)
  ...
  return (
    <div
      ...
      className={`${viewed && !applied ? 'bg-gray-50 dark:bg-gray-800/60' : 'bg-white dark:bg-gray-800'} rounded-2xl border border-gray-200 dark:border-gray-700 p-5 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 transition-all group cursor-pointer`}
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <button onClick={openDetails} className="text-left">
        ...
        <div className="flex flex-shrink-0 flex-wrap justify-end gap-1.5">
          ...
          {isNew && !viewed && !applied && (
            <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800">
              Nova
            </span>
          )}
        </div>
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1.5 text-xs text-gray-500 dark:text-gray-400 mb-4">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          {timeAgo && <span className="text-gray-400 dark:text-gray-500">{timeAgo}</span>}
        </div>
      </div>
